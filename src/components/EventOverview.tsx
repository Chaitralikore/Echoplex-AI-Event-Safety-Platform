...
const EventOverview: React.FC = () => {
  ...
  useEffect(() => {
    // Reduced from 2s to 30s for performance with large datasets
  ...
  useEffect(() => {
    fetchZoneData();
    // Reduced from 5s to 60s for performance with 10k+ attendees
    const interval = setInterval(fetchZoneData, 60000);
    return () => clearInterval(interval);
  }, []);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-cyan-900/60 to-cyan-800/40 rounded-xl p-6 border border-cyan-700/30 backdrop-blur-sm">
          ...
          <div className="w-full bg-cyan-900/30 rounded-full h-3">
            <div
              className={`h-3 rounded-full transition-all duration-500 ${crowdDensity >= 0.8 ? 'bg-gradient-to-r from-red-500 to-red-600' :
        ...
        <div className="bg-gradient-to-br from-amber-900/60 to-amber-800/40 rounded-xl p-6 border border-amber-700/30 backdrop-blur-sm">
          ...
          <div className="w-full bg-amber-900/30 rounded-full h-3">
            <div
              className={`h-3 rounded-full transition-all duration-500 ${weatherRisk > 0.5 ? 'bg-gradient-to-r from-red-500 to-red-600' :
          ...
          {aiInsights.map((insight, index) => (
            <div key={index} className="bg-slate-700/30 rounded-lg p-4 border border-slate-600/30">
              <div className="flex items-center justify-between mb-3">
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${insight.priority === 'High' ? 'bg-red-900/40 text-red-400' :
              ...
              <div key={zone.id} className="bg-slate-700 rounded-xl p-5 border border-slate-600">
                ...
                <div className="mb-4">
                  <div className="w-full bg-slate-600 rounded-full h-4 overflow-hidden">
                    <div
                      className={`h-full transition-all duration-500 ${risk === 'high'
                          ? 'bg-red-500'
                          : risk === 'medium'
                            ? 'bg-amber-500'
                            : 'bg-emerald-500'
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold uppercase ${risk === 'high'
            ...
            {[
              ...
              <div key={drone.id} className="bg-slate-700/30 rounded-lg p-4 border border-slate-600/30">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${drone.status === 'Patrolling' ? 'bg-emerald-900/40 text-emerald-400' :
                ...
                <div className="flex items-center space-x-3">
                  <div className="flex-1">
                    ...
                    <div className="w-full bg-slate-600/50 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full transition-all duration-300 ${drone.battery > 50 ? 'bg-gradient-to-r from-emerald-500 to-emerald-600' :
